...
{
    public class SubSonicDatabase : SubSonic.Repository.SimpleRepository, IDatabase
    {
        private string connectionStringName;

        public SubSonicDatabase(string connectionString)
        ...
        public bool Delete<T>(Expression<Func<T, bool>> expression, int timeout = 15, string tableName = null) where T : class, new()
        {
            ...
            var orm = new CodingHorror(command.Statement, command.Parameters.ToObjectArray(true));
            return orm.Execute() > 0;
        }
            ...
            using (var sharedConnectionScope = new SharedDbConnectionScope())
            {
                try
                {
                    foreach (var unitOfWork in unitOfWorks)
                    {
                        ...
                        var orm = new CodingHorror(unitOfWork.Statement, unitOfWork.Parameters.ToObjectArray(true));
                        orm.Execute();
                    }
        ...
        {
            var orm = new CodingHorror(sql, parameters.ToObjectArray(true));
            return orm.Execute() > 0;
        }
            ...
            where T : class, new()
        {
            ...
            if (tableName.IsNullOrEmpty())
                ...
                command = DynamicQuery.GetPageQuery<T>(tableName, null, take, skip, timeout, this.DbOrmType);
            if (command == null) return null;
            var orm = new CodingHorror(command.Statement, command.Parameters.ToObjectArray(true));
            return orm.ExecuteTypedList<T>();
        }
